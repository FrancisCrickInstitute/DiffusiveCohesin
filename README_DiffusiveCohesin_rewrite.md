...
Using the same random seed is expected to reproduce the same trajectory only when using the same LAMMPS version, the same executable, the same compiler and compiler flags, the same operating system, the same input files, and the same processor configuration.

Across different operating systems, CPUs, compilers, or compiler versions, the same seed should be interpreted as reproducing the same simulation protocol, not necessarily the exact same bead-by-bead trajectory. For example, a macOS build using AppleClang and a Linux build using GCC may produce trajectories that diverge after many time steps because of small floating-point round-off differences.
